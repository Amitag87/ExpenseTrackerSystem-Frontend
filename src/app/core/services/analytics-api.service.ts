...
export class AnalyticsApiService {
  ...
  incomeExpenseTrend(userId: number): Observable<any[]> {
    return this.http.get<any[]>(`${apiConfig.analytics}/trends/income-expense/${userId}`);
  }

  savingsRateTrend(userId: number): Observable<number[]> {
    return this.http.get<number[]>(`${apiConfig.analytics}/trends/savings-rate/${userId}`);
  }

  dailyTrend(userId: number): Observable<number[]> {
    return this.http.get<number[]>(`${apiConfig.analytics}/trends/daily/${userId}`);
  }
}
