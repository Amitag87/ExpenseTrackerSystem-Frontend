...
export class BudgetsComponent implements OnInit {
  ...
  calculateProgress(budget: Budget): number {
    const p = this.progressMap.get(budget.budgetId);
    if (p) return p.percentageUsed;
    if (!budget.limitAmount) return 0;
    return ((budget.spentAmount || 0) / budget.limitAmount) * 100;
  }
